...
/**
 * The BYOK model-source chain: list connectable sources, run the hosted
 * browser credential handoff, bind the connection, and list the
 * certification-backed deployments arms are enabled from.
 *
 * HARD INVARIANT (docs/MCP_SERVER.md): the MCP server NEVER accepts a raw
...
export const listSourcesTool = {
    ...
    description: "List the model sources this tenant can connect (GET /v1/model-source-profiles). " +
        "BYOK-only by construction: each entry carries its protocol/auth profile versions " +
        "and endpoint policy. No credential material is involved.",
...
export const createSourceConnectionTool = {
    ...
    description: "Bind a BYOK source connection from a COMPLETED handoff intent (POST /v1/source-connections). " +
