...
expectFindings("every name outside the surface fails", expectedToolNames.map((name) => `${name}_x`), expectedToolNames.length * 2);

// Asserted as properties, not as a count: a transcribed number goes stale on
// the release that changes the surface, and the count is not what makes the
// pin trustworthy. A gutted, duplicated, or unsorted pin file is.
if (expectedToolNames.length === 0) failures.push("pinned surface is empty");
if (new Set(expectedToolNames).size !== expectedToolNames.length) failures.push("pinned surface repeats a name");
if (expectedToolNames.join("\u0000") !== [...expectedToolNames].sort().join("\u0000")) {
  failures.push("pinned surface is not in sorted order");
}
if (expectedToolNames.some((name) => !/^solver_[a-z_]+$/.test(name))) {
  failures.push("pinned surface carries a name that is not a solver_ tool");
}
...
process.stdout.write(`tool-surface drift negatives ok (6 cases, ${expectedToolNames.length} pinned names)\n`);
